...
describe('PaginationInterceptor', () => {
  ...
  it.each([
  ...
  ])('should transform %O %i and add extra information limit=%s page=%s', async (items, totalItems, limit, page, pages) => {
      ...
      links: {
        ...
        previous:
          page && +page > 1
            ...
            : undefined,
        next:
          Number.parseInt(page as string, 10) || defaultPage < pages
            ? expect.stringContaining('http://localhost/api')
            : undefined,
        last:
          pages > 1
    ...
    if (result.links.next) {
      ...
      expect(next.searchParams.get('page')).toBe(
        String((page ? Number.parseInt(page, 10) : defaultPage) + 1)
      );
    }
